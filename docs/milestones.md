...
### Data integrity gaps
- **Guardian dedup by email** — **addressed go-forward by P6 guardian-identity Slice A (2026-06-10).** `Guardian` is now canonical and 1:1 with `ParentAccount` via `Guardian.parent_account` (OneToOneField); `resolve_guardian_for_account` is called at draft initiation and again on every verified save, and `approve_application` reuses `application.guardian` instead of creating a new row unconditionally. A parent's children now share ONE `Guardian`, so sibling-discount linkage and the single Invoice Ninja client per parent both hold for any registration opened after Slice A landed. — Read-through propagation of the denormalized `guardian_*` snapshot columns on `RegistrationApplication` is Slice B; admin-initiated email-change workflow is Slice C; parent self-service email change is deferred.
