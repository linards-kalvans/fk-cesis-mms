...
- admin **review consolidation delivered (P7 Slice C-i, 2026-06-14)** — the registration review+edit flow now lives on the Django admin change page (panels + agreement/training-group modules + a top action bar), with status-aware one-click quick actions on the changelist; the bespoke custom review queue/detail views, URLs, and templates were removed. **Remaining = P7 Slice C-ii**: cross-links (application↔member↔agreement↔billing), sync-health badges/filters, search/filter polish, document active-vs-replaced UX, training-group de-duplication.
