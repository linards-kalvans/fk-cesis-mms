...
- admin **review consolidation delivered (P7 Slice C-i, 2026-06-14)** — the registration review+edit flow now lives on the Django admin change page (panels + agreement/training-group modules + a top action bar), with status-aware one-click quick actions on the changelist; the bespoke custom review queue/detail views, URLs, and templates were removed. **Remaining = P7 Slice C-ii** (admin flow polish). User-prioritised items (2026-06-14): (a) move the Registrations app to the **top** of the admin left-side menu; (b) show the **agreement status** as a column in the applications changelist; (c) **one-click confirm** of a billing record — a button near the top of the BillingRecord change page AND directly from the billing-records list (replacing the open→dropdown→save dance). Plus the original C-ii scope: cross-links (application↔member↔agreement↔billing), sync-health badges/filters, search/filter polish, document active-vs-replaced UX, training-group de-duplication.
