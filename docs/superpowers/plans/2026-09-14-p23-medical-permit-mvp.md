# P23 Medical Permit MVP — Implementation Plan

**Goal:** Let parents upload a child's medical certificate and let staff confirm receipt, track validity through September 30 of the following year, and view status across parent portals, registration admin, and the family hub. No OCR, no automatic expiry enforcement, no reminder emails.
...
| `apps/documents/migrations/0007_medicalpermit.py` | Create `MedicalPermit` table; depend on `documents/0006`, `registrations`, `members`, `core` (User). |
| `apps/core/models.py` + `apps/core/migrations/0012_alter_auditevent_action.py` | Add six `AuditEvent.Action` choices: `MEDICAL_PERMIT_UPLOADED`, `MEDICAL_PERMIT_REPLACED`, `MEDICAL_PERMIT_CONFIRMED`, `MEDICAL_PERMIT_CONFIRMATION_CLEARED`, `MEDICAL_PERMIT_PREVIEWED`, `MEDICAL_PERMIT_DOWNLOADED`. |
| `apps/documents/medical_permits.py` | Service layer: `validate_medical_permit_upload`, `_write_permit_file`, `replace_medical_permit`, `upload_application_medical_permit`, `upload_member_medical_permit`, `confirm_medical_permit`, `clear_medical_permit_confirmation`, `attach_application_medical_permit`, `medical_permit_status`, `medical_permit_status_label`, `can_parent_upload_application_medical_permit`. |
...
| `templates/parent_ui/includes/medical_permit_card.html` | Reusable card partial: status label, preview/download/upload, `fk-document-card` grammar (identical to `document_card.html`), `Neobligāti` label, hidden native input + full-width label control, uploaded filename/replace/preview/download. **Async upload refinement:** normal upload auto-starts on file selection (no manual submit button). JS-visible workspace has no external permit form; portal uses a standalone multipart form. No-JS fallback: portal card includes a standalone multipart form; editable workspace includes a `<noscript>` form after wizard close (never nested). |
| `templates/registrations/application_workspace.html` | Medical permit card rendered immediately after parent ID, child ID, portrait cards (step 1). `Neobligāti` visible; no submit/wizard gate. **Async upload:** upload auto-starts on file selection — no manual submit button. No external permit form in the JS-visible DOM. No-JS fallback: a `<noscript>` form rendered after the wizard close, never nested inside the wizard `<form>`. |
| `templates/registrations/parent_portal.html` | Medical permit card as a dedicated full-width row after the progress bar and before the CTA buttons, for every policy-allowed application (draft/fix_requested/submitted/approved). Status label + August-starting expiry warning (`data-medical-permit-warning`). Upload auto-starts on file selection — no manual submit button. Portal cards use a standalone multipart form (also serves as the no-JS fallback). |
...
| `tests/documents/test_medical_permit_model.py` | Model defaults, constraints, cascade. |
...
class MedicalPermit(TimeStampedModel):
    ...
    file = FileField(upload_to=medical_permit_upload_to, storage=PrivateDocumentStorage(), blank=True)
...
```python
def test_anonymous_preview_is_redirect_to_registration_start(client, application):
    ...
...
- Create: `apps/documents/migrations/0007_medicalpermit.py`
- Modify: `apps/core/models.py`
- Create: `apps/core/migrations/0012_alter_auditevent_action.py`
...
`application_medical_permit_upload` and `member_medical_permit_upload` — POST only, validate ownership, call service, return JSON `201` (success) or `400` (validation error). `medical_permit_preview` and `medical_permit_download` — enforce ownership, stream file with correct `Content-Type` and disposition; anonymous → redirect to registration start; foreign/missing/file-less → `404`.
...
path("portal/members/<int:member_id>/medical-permit/upload/", ...),
path("portal/medical-permits/<int:permit_id>/preview/", ...),
path("portal/medical-permits/<int:permit_id>/download/", ...),
```

- [x] **Step 7: Render medical permit card in parent templates.**

Reusable partial `templates/parent_ui/includes/medical_permit_card.html` — shared `fk-document-card` grammar matching identity-document and portrait cards. One partial consumed by both workspace and portal.

`application_workspace.html`: render the card immediately after parent ID, child ID, and portrait document cards (step 1 of the wizard). `Neobligāti` label visible. No submit/wizard gate. **Async upload:** upload auto-starts on file selection — no manual submit button. No external permit form in the JS-visible DOM. Native file input hidden via `.fk-visually-hidden`; upload triggered via full-width `<label>`.

`parent_portal.html`: render the card as a dedicated full-width row after the progress bar and before the CTA buttons, for every policy-allowed application (draft/fix_requested/submitted/approved). Status label + August-starting expiry warning (`data-medical-permit-warning`). Upload auto-starts on file selection — no manual submit button. Portal cards use a standalone multipart form (also serves as the no-JS fallback).
...
### Async upload refinement (post-MVP polish)

The original MVP shipped with a manual-submit upload pattern (the card rendered a multipart form the user had to submit). The refinement replaces this with immediate auto-upload on file selection, mirroring the existing identity-document upload UX.

**Key behavioral changes:**
- Normal upload triggers immediately on `<input type="file">` change — no separate submit button.
- The existing POST endpoints (`/applications/<id>/medical-permit/upload/`, `/portal/members/<id>/medical-permit/upload/`) are unchanged: same authorization, same HTTP 201 JSON response (`filename`, `status`/`status_label`, `preview_url`, `download_url`).
- The browser updates the permit card in place after upload completes — no full page reload, no OCR dependency. Errors render as Latvian inline alerts.
- No-JS fallback preserved: portal card keeps its standalone multipart form; editable workspace gets a `<noscript>` form after the wizard close (never nested).
- No schema, migration, security policy, OCR, email, billing, or status rule changes.

**Verification:** full suite 2632 passed; ruff/mypy/makemigrations clean. LAN acceptance COMPLETE — signed off 2026-09-18.
