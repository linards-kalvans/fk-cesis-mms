...
- Full repo verification after Slice D landing: `uv run pytest -q` → `794 passed`, `uv run ruff check .` → passed, `uv run mypy .` → passed (143 source files).
- Manual LAN verification on `http://192.168.3.245:8000/applications/<id>/` is pending — to be recorded once the user completes the phone/desktop check.
