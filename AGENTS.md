...
- **P6 Slice A delivered — local billing domain + sibling-discount engine (2026-06-07)**
  ...
  - Full-repo verification after Slice A: `uv run pytest -q` → **1052 passed**, `uv run ruff check .` → clean, `uv run mypy .` → clean (219 source files). LAN smoke per the plan's final checklist still pending.
