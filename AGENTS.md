...
- **P6 Slice B delivered — Invoice Ninja push integration (2026-06-08)**
  ...
  - Full-repo gate after Slice B: `uv run pytest -q` → **1078 passed**, `uv run ruff check .` → clean, `uv run mypy .` → clean (230 source files). LAN smoke per the plan's final checklist still pending.
