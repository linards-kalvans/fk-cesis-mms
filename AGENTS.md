...
- **P6 Slice B delivered — Invoice Ninja push integration (2026-06-08)**
  ...
  - Full-repo gate after Slice B: `uv run pytest -q` → **1080 passed**, `uv run ruff check .` → clean, `uv run mypy .` → clean (230 source files). LAN smoke per the plan's final checklist still pending.
  - **Deferred to Slice C (hardening, flagged in final review, not blocking):** `ensure_product`/`ensure_client` have a narrow crash-window dedup gap — if the IN POST succeeds but the Django `save()` of the external id fails, a retry can create a duplicate product/client in IN (the invoice path is already protected by deterministic-number recovery). Fix needs a lookup key: product by `product_key`, client by a stored identifier (e.g. `custom_value1=guardian.pk`). No Django-side data loss; `external_status` surfaces the failure for manual recovery.
