...
## Current Status
**Tasks 1–6 complete in current worktree, P1 + P2 are complete, P3 is signed off (live validation evidence in `docs/p3_tiny_idp_validation.md`), P4 is complete, and P5 Slices A–D are delivered (full approval-to-agreement flow including DocuSeal-backed e-signature).** Registration workflow is usable for LAN acceptance testing; admin review queue, member creation baseline, guardian-email-first verified registration gate, OCR-backed parent/admin review flow, and the agreement lifecycle (generate → sent → DocuSeal-signed/manual, with void + regenerate) are operational.
...
- **P5 Slice A delivered — admin review: inline doc preview + approval-ready inspection (2026-05-27)**
  ...
  - Full repo verification after Revision A + Slice A.1 landed: `uv run pytest -q` → `891 passed`, `uv run ruff check .` → passed, `uv run mypy .` → passed (155 source files). Manual LAN re-verification PENDING — user to confirm at `http://192.168.3.245:8000/admin/review/applications/<id>/` that the admin shell looks like Django admin, thumbnails open the lightbox, replaced-image docs open as `<img>`, and the three review-action emails contain the application or portal URL.
...
- **P5 Slice B delivered — training-group assignment workflow (2026-05-28)**
  ...
  - Manual LAN verification on `http://192.168.3.245:8000/admin/review/applications/<id>/` PENDING — user to confirm picker contents, approve+assign in one click, post-approval reassign/clear, inactive marker, conditional email line.
